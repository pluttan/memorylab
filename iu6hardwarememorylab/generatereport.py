...
Radix-Counting показывает лучшие результаты на больших массивах.""",
        ...
        7: {
            "title": "Исследование самомодифицирующегося кода (JIT vs Branching)",
            "goal": "Исследование влияния самомодифицирующегося кода и JIT-компиляции на производительность рендеринга.",
            "problem": """Современные процессоры используют конвейерную обработку (pipelining). При условном переходе 
процессор должен предсказать, какая ветвь будет выполнена. При неверном предсказании (branch misprediction):
- Конвейер очищается (pipeline flush)
- Теряется 10-20 тактов процессора
- Производительность падает

В критических циклах рендеринга это может значительно снизить FPS.""",
            "method": """Эксперимент проводится на движке DOOM, сравнивая два режима отрисовки колонок:
1) JIT режим — использует loop unrolling (развёртывание цикла 8x) или настоящий самомодифицирующийся код на x86_64 Linux
2) Branch режим — использует volatile указатели, что форсирует перезагрузку данных каждую итерацию

Режимы автоматически переключаются каждую секунду для честного сравнения.""",
            "analysis": """График показывает время рендеринга кадра для двух режимов. JIT режим (зелёный) 
демонстрирует более стабильное и низкое время кадра за счёт:
- Меньшего количества проверок условия цикла (8x меньше)
- Кеширования указателей в регистрах
- Лучшей предсказуемости переходов

Speedup показывает отношение времени Branch к времени JIT — чем выше, тем эффективнее JIT.""",
            "img_name": "exp7_doom_jit_benchmark",
            "param_labels": {
                "source": "Источник данных",
                "total_entries": "Количество записей",
            },
            "result_labels": {
                "speedup": "Ускорение (JIT/Branch)",
                "jit_avg_ms": "Среднее время кадра (JIT)",
                "branch_avg_ms": "Среднее время кадра (Branch)",
    ...
    def _format_value(self, key: str, value: Any) -> str:
        ...
        if "_us" in key:
            ...
            if isinstance(value, (int, float)):
                return f"{value:.4f} мс"
        elif "_ratio" in key or key == "speedup":
            if isinstance(value, (int, float)):
                return f"{value:.2f}x"
...
+ Исследование самомодифицирующегося кода (JIT vs Branching).
...
'''
        ...
        # Добавляем PMU график если есть
        pmu_img_name = f"exp{num}_pmu_metrics"
        pmu_img_path = self.img_dir / f"{pmu_img_name}.png"
        
        if pmu_img_path.exists():
            section += f'''=== PMU метрики

#рис(image("img/{pmu_img_name}.png", width: размер({self.IMAGE_WIDTH})))[PMU метрики эксперимента {num}] <exp{num}_pmu>
