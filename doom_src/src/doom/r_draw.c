...
// JIT self-modifying code comparison
#include "r_jit.h"
...
//
// Modified for JIT benchmark comparison
//
void R_DrawColumn(void)
{
    ...
    // JIT Benchmark: choose rendering path based on mode
    jit_frame_calls++; // Count calls for this frame

    if (jit_mode_enabled)
    {
#if defined(__x86_64__) && defined(__linux__)
        // Real JIT mode for x86_64 Linux: use self-modifying code
        jit_draw_column_fn jit_fn;

        // Generate/update JIT code if colormap changed
        R_JIT_GenerateDrawColumn(dc_colormap);
        jit_fn = R_JIT_GetDrawColumn();

        if (jit_fn)
        {
            // Call JIT-compiled function with baked colormap address
            jit_fn(dest, dc_source, dc_colormap, count, fracstep, frac);
        }
        else
#endif
        {
            // Fallback: 8x loop unrolling for other platforms
            lighttable_t *colormap = dc_colormap;
            byte *source = dc_source;

            // Unrolled loop - process 8 pixels at a time
            while (count >= 7)
            {
    ...
    {
        // Branching mode: standard path with pointer indirection every iteration
        // This simulates code that must reload the colormap pointer
        do
        {
            // Force pointer reload from global (no caching)
            volatile lighttable_t *colormap = dc_colormap;
