...
// JIT self-modifying code benchmark
#include "r_jit.h"
...
boolean G_Responder(event_t *ev)
{
    ...
    // J key: Toggle JIT rendering mode and print statistics
    if (ev->type == ev_keydown && ev->data1 == 'j')
    {
        printf("JIT KEY PRESSED!\n");
        R_JIT_Toggle();
        R_JIT_PrintStats();
        return true;
    }

    // K key: Toggle auto-switch mode (every 1 second)
    if (ev->type == ev_keydown && ev->data1 == 'k')
    {
        R_JIT_ToggleAutoSwitch();
        return true;
    }
